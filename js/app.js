...
const App = {
    ...
    cacheElements() {
        this.elements = {
            ...
            // Selection Guide
            selectionGuide: document.getElementById('selection-guide'),

            // Sticky Action Bar
            stickyActionBar: document.getElementById('sticky-action-bar'),
            stickyBarInfo: document.getElementById('sticky-bar-info'),
            stickySelectAllBtn: document.getElementById('sticky-select-all-btn'),
            stickyDownloadBtn: document.getElementById('sticky-download-btn')
        };
    },
    ...
    bindEvents() {
        ...
        // Sticky Action Bar 按鈕
        this.elements.stickySelectAllBtn.addEventListener('click', () => {
            this.toggleSelectAll();
        });

        this.elements.stickyDownloadBtn.addEventListener('click', () => {
            this.downloadSelectedImages();
        });
    },
    ...
    async handlePDFUpload(file) {
        ...
        try {
            ...
            this.elements.totalPagesCount.textContent = this.state.totalPages;
            ...
            // 捲動到框選區域
            this.elements.stepRegion.scrollIntoView({ behavior: 'smooth' });
    ...
    applyAdjustments(sourceCanvas, rotation, scale, outputWidth, outputHeight) {
        ...
        // 使用等比縮放（cover 模式），避免圖片變形
        const uniformScale = Math.max(outputWidth / sourceCanvas.width, outputHeight / sourceCanvas.height) * scaleFactor;
    ...
    endSelection(e) {
        ...
        this.showToast('區域選取完成！可調整角度和縮放，全部擷取完後按下方按鈕', 'success');
    },
    ...
    async updateRegionPreview() {
        ...
        // 使用當前已渲染的 canvas 避免重複渲染（效能優化）
        // 如果當前 canvas 有內容則直接使用，否則重新渲染
        let sourceCanvas = this.elements.pdfCanvas;
        if (sourceCanvas.width === 0 || sourceCanvas.height === 0) {
            const pageResult = await PDFHandler.extractPageAsImage(pageNum, 1.5);
            sourceCanvas = pageResult.canvas;
        }
        const pageResult = { canvas: sourceCanvas };
    ...
    renderCropPreview() {
        ...
        const previewWidth = 260;  // 預覽寬度（接近實際輸出 413px 的比例）
        ...
        // 儲存狀態，移動到中心點，旋轉並縮放
        // 使用等比縮放（cover 模式），避免圖片變形
        const uniformScale = Math.max(previewWidth / sourceCanvas.width, previewHeight / sourceCanvas.height) * scaleFactor;
    ...
    removePDF() {
        ...
        this.state.currentPage = 1;
        this.state.pageRegions = {};
        this.state.regionAdjustments = {};
        this.state.currentCropCanvas = null;
        this.state.images = [];
        this.state.selectedImages.clear();
        this.state.selectionGuideShown = false;
    ...
    async extractAllImages() {
        try {
            ...
            if (pagesToExtract.length === 0) {
                ...
                this.showToast('沒有選取任何頁面的照片區域', 'warning');
                return;
            }
    ...
    updateImageTransform(index) {
        ...
        // 使用等比縮放（cover 模式），避免圖片變形
        const uniformScale = Math.max(outputWidth / img.canvas.width, outputHeight / img.canvas.height) * scaleFactor;
        ctx.translate(outputWidth / 2, outputHeight / 2);
        ctx.rotate(radians);
        ctx.scale(uniformScale, uniformScale);
        ctx.drawImage(img.canvas, -img.canvas.width / 2, -img.canvas.height / 2);
     ...
     */
    initTheme() {
        const savedTheme = localStorage.getItem('pdf-extractor-theme');
        if (savedTheme === 'light') {
    ...
    toggleTheme() {
        ...
        localStorage.setItem('pdf-extractor-theme', isLight ? 'light' : 'dark');
    },
     ...
     */
    showSelectionGuide() {
        if (this.state.selectionGuideShown) return;
        const guide = this.elements.selectionGuide;
        if (guide) {
    ...
    hideSelectionGuide() {
        ...
        if (guide) {
            ...
            this.state.selectionGuideShown = true;
        }
    },
    ...
    /**
     * 更新 Sticky Action Bar
     */
    updateStickyBar() {
        const count = this.state.selectedImages.size;
        const total = this.state.images.length;
        const isResultsVisible = !this.elements.stepResults.hidden;

        if (isResultsVisible && total > 0) {
            this.elements.stickyActionBar.hidden = false;
            document.body.classList.add('has-sticky-bar');
            this.elements.stickyBarInfo.textContent = `已選取 ${count} / ${total} 張照片`;
            this.elements.stickyDownloadBtn.disabled = count === 0;
            this.elements.stickyDownloadBtn.innerHTML = `⬇️ 下載選取的圖片 (${count})`;

            const allSelected = count === total;
            this.elements.stickySelectAllBtn.textContent = allSelected ? '取消全選' : '全選';
        } else {
            this.elements.stickyActionBar.hidden = true;
